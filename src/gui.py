...
class TrainingGUI(ctk.CTk):
    def __init__(self):
        ...
        self.datatype_menu = ctk.CTkOptionMenu(self.datagen_frame, values=["bouncing", "rigid", "domino", "inclined_plane"], font=self.huge_font, width=180, command=self._on_physics_type_changed)
        ...
        try:
            ...
            if env_type == "bouncing":
                ...
                generate_bouncing_rigid_data(data_dir=data_dir, n_trajectories=n_traj, width=res, height=res,
                                             n_balls_min=bmin, n_balls_max=bmax, speed_min=smin, speed_max=smax,
                                             progress_cb=cb)
            elif env_type == "domino":
                generate_domino_data(data_dir=data_dir, n_trajectories=n_traj, width=res, height=res,
                                     n_dominoes_min=bmin, n_dominoes_max=bmax, progress_cb=cb)
