...
def _polygon_points(center_x, center_y, radius, n_sides, angle_offset=0.0):
    angles = angle_offset + np.linspace(0.0, 2.0 * np.pi, n_sides, endpoint=False)
    points = np.stack(
...
def _draw_rigid_body(img, body, height):
    center = (round(body["x"] * _SUBPIX), round((height - body["y"]) * _SUBPIX))
    radius = round(body["radius"] * _SUBPIX)
    shape = body["shape"]

    if shape == "circle":
        cv2.circle(img, center, radius, body["mat"]["color"], -1,
    ...
    if shape == "square":
        ...
        cv2.fillPoly(img, [np.round(points).astype(np.int32)], body["mat"]["color"],
                     lineType=cv2.LINE_AA, shift=SUBPIX_BITS)
        return

    if shape == "triangle":
        points = _polygon_points(center[0], center[1], radius, 3, angle_offset=-np.pi / 2.0)
        cv2.fillPoly(img, [points], body["mat"]["color"], lineType=cv2.LINE_AA, shift=SUBPIX_BITS)
        return

    if shape == "hexagon":
        points = _polygon_points(center[0], center[1], radius, 6, angle_offset=np.pi / 6.0)
        cv2.fillPoly(img, [points], body["mat"]["color"], lineType=cv2.LINE_AA, shift=SUBPIX_BITS)
        return

    raise ValueError(f"Unsupported rigid body shape: {shape}")


def _make_ball(width, height, speed_min, speed_max):
    """Create one rigid body with a random material, shape, size, position and velocity."""
    ...
    if shape == "circle":
        ...
        area = (3.0 * np.sqrt(3.0) / 4.0) * radius ** 2
    else:
...
def generate_bouncing_data(data_dir="data/rigid_bouncing", n_trajectories=5000, max_frames=100,
                           width=WIDTH, height=HEIGHT, n_balls_min=1, n_balls_max=5,
                           speed_min=3.0, speed_max=8.0, n_substeps=4, progress_cb=None):
    """Generate the bouncing rigid-body dataset. Resolution must be a multiple of 8 to match
    ...
    for i in tqdm(range(n_trajectories), desc="Generating RGB Physics Envs"):
        ...
        for frame in range(max_frames):
            # Render RGB Image (rigid bodies drawn over a white background). Fixed-point sub-pixel center +
            # anti-aliasing so the continuous physics position survives into the pixels (see SUBPIX_BITS).
            img = np.ones((height, width, 3), dtype=np.uint8) * 255
            for b in balls:
                _draw_rigid_body(img, b, height)
