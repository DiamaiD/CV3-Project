...
def _make_plane(width,height,number:int):
    ...
    if number == 1:
        ...
        x2 = (width // 2)-10  # etwas weiter nach rechts gestreckt
...
def _spawn_balls(n_balls, plane, width, height, speed_min, speed_max, max_tries=1000):
...
def generate_trichter_data(data_dir="data/trichter", n_trajectories=5000, max_frames=100,
                           width=WIDTH, height=HEIGHT, n_balls_min=1, n_balls_max=5,
                           speed_min=3.0, speed_max=8.0, n_substeps=4, progress_cb=None):
    """Generate the trichter dataset. Resolution must be a multiple of 8 to match
    ...
    for i in tqdm(range(n_trajectories), desc="Generating RGB Physics Envs"):
        ...
        plane = _make_plane(width,height,1)
        plane2 = _make_plane(width,height,2)
        ...
        for frame in range(max_frames):
            ...
            cv2.line(
                img,
                (plane2["x1"], height - plane2["y1"]),
                (plane2["x2"], height - plane2["y2"]),
                (0, 0, 0), 1, cv2.LINE_AA
            )
            ...
            for _ in range(n_substeps):
                ...
                resolve_ball_plane_collisions(balls,plane2)
                for b in balls:
                    radius = b["radius"]
                    fric = b["mat"]["friction"]
